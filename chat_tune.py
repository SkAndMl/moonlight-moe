...
logger.info(f"[chat] dtype: {autocast_dtype}")
training_cfg = config.TrainingConfig(device=device, max_lr=7.5e-5, min_lr=7.5e-6)
...
# setup dataloaders
train_dl, test_dl = get_dataloaders(Path("sft_data/chat/train"), Path("sft_data/chat/test"), training_cfg)
...
logger.info(f"[chat] model params: {sum(p.numel() for p in model.parameters()):,}")
logger.info(f"[chat] decay params: {sum(p.numel() for p in param_groups['decay']):,}")
logger.info(f"[chat] non_decay params: {sum(p.numel() for p in param_groups['non_decay']):,}")
...
optimizer = torch.optim.AdamW(
    ...
    betas=(0.9, 0.95),
    eps=1e-8,
    fused=device.startswchath("cuda")
)

total_steps = len(train_dl) // training_cfg.accumulation_steps
warmup_steps = int(0.10 * total_steps)
logger.info(f"[chat] total_steps: {total_steps} | warmup_steps: {warmup_steps}")

wandb.init(
    project=f"gpt-moe-chat-{datetime.now().strftime('%d_%m_%Y')}",
    config={
...
CKPT_DIR = pathlib.Path("checkpoints/chat")
...
def construct_mask(y: torch.Tensor) -> torch.Tensor:
    ...
    def _construct_mask(token_id, occurence="first", op=operator.le):
        _mask: torch.Tensor = y == token_id
        count = _mask.long().cumsum(dim=1)
        
        if occurence == "first":
            target_mask = count == 1
        elif occurence == "last":
            target_mask = count == count[:, -1]
        elif isinstance(occurence, int):
            target_mask = count == occurence
        else:
            raise ValueError()

        indices = target_mask.long().argmax(dim=1).unsqueeze(1)
        mask = op(prompt_mask[:y.shape[0], :], indices)
        return mask

    content_mask = _construct_mask(ASSISTANT_TOKEN_ID, "last", operator.le)
    eot_mask = _construct_mask(EOT_TOKEN_ID, "first", operator.gt)
...
for step in range(total_steps):
    ...
    logger.info(f"[chat] step: {step + 1:>5} | loss: {loss_accum:.4f} | lr: {lr:2e} | norm: {norm.item():.4f} | token throughput: {token_throughput:.4f}")
...
upload_to_hf(pathlib.Path("checkpoints/chat/chat_best.pt"), "SkAndMl/moonlight-moe-chat")